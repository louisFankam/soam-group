...
export default async function ListeCategoriePage() {
  const nomModule = "blog/categories";
  ...
  return (
    <div className="max-w-4xl space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <Link
          href={`/superadmin/blog/categories/nouveau`}
      ...
      <div className="bg-card border border-card-border rounded-2xl overflow-hidden">
        <ul className="divide-y divide-card-border">
              ...
              <li key={String(donnees.id)} className="px-5 py-3.5 flex items-center justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground truncate">
                    {String(donnees.group)} — /{String(donnees.slug)}
                  </p>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  <Link
                    href={`/superadmin/blog/categories/${donnees.id}`}
          ...
          {lignes.length === 0 && (
            <li className="px-5 py-8 text-sm text-muted-foreground">Aucune catégorie.</li>
          )}
        </ul>
      </div>
    </div>
